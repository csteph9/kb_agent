...
The ingestion system supports drop-in Gmail, RSS/Atom, ICS calendar and
website connectors, configured sources, one generic timer, durable retries and
shared knowledge writes. See [the ingestion guide](docs/ingestion.md) for the
connector contract, source examples and operational details.

**Existing KB server:** deploy from a separate application checkout with
`sudo bash deploy-ingest.sh`. The script migrates Gmail state, preserves OAuth
files and source configuration, and switches the ingestion timer. Telegram and
the KB repository remain in place.
...
Install a current Node.js release. Node.js 22.16 or newer is required for the ingestion runtime.
...
sudo -u knowledge -H npm ci --omit=dev --ignore-scripts
```

The application includes package.json and package-lock.json. Keep both when
deploying updates.
...
The GMAIL_* settings are legacy migration inputs. After migration, edit
the source JSON under config/sources/ to enable Gmail, change its query, or set
attachment/work limits. Credentials remain outside Git; processed IDs and jobs
live in the private ingestion SQLite database. See
[the ingestion guide](docs/ingestion.md) for all source settings.
...
pipeline. `knowledge-ingest.timer` checks for due work every minute. Each source
has its own interval. Leave the legacy Gmail timer disabled.

After Gmail configuration has been migrated or a new source is configured:
...
# Optional:
# sudo systemctl status knowledge-ingest.timer --no-pager -l
...
``` bash
sudo journalctl -u knowledge-ingest.service -n 100 --no-pager
```
...
## Optional Gmail and other ingestion sources

Gmail is a connector in the shared ingestion pipeline. For an existing
installation, run the ingestion deployment script described at the top of this
README. For a new installation, copy a source example from
`config/sources/personal-gmail.example.json` to `personal-gmail.json`, configure
OAuth credentials in its stateDir, then authorize the configured account:
...
Set enabled to true in the source configuration, then run it manually and enable
the generic timer:

```bash
sudo -u knowledge -H node /opt/knowledge-agent/ingest/cli.js run personal-gmail
sudo systemctl enable --now knowledge-ingest.timer
```

Use [docs/ingestion.md](docs/ingestion.md) for Google credential placement,
legacy migration, RSS/calendar/website setup, retries, and custom connectors.
...
``` bash
sudo systemctl status knowledge-ingest.service --no-pager -l
sudo journalctl -u knowledge-ingest.service -n 100 --no-pager
sudo -u knowledge -H ls -l /opt/knowledge-agent/gmail
sudo -u knowledge -H git -C /home/knowledge/repo status
```

Check that the source JSON has enabled set to true and its credentials
directory contains credentials.json and token.json. Run
`node /opt/knowledge-agent/ingest/cli.js check personal-gmail` as knowledge,
then inspect `ingest/cli.js status` for retryable jobs.
...
-   Optional Gmail ingestion has `knowledge-ingest.timer` active,
    valid OAuth files under `/opt/knowledge-agent/gmail`, and a clean
    ingestion state database with no unresolved failures.
